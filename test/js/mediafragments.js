var MediaFragments = (function(window) {
  ...
  var logger = { // can be replaced by another object calling setLogger(logger)
    'warn': logWarning
  }
  ...
  var dimensions = {
    t: function(value) {          
      ...
      // @dsm: changed to accept one-digit components as well
      // var npt = /^((npt\:)?((\d+\:(\d\d)\:(\d\d))|((\d\d)\:(\d\d))|(\d+))(\.\d*)?)?$/;
      var npt = /^((npt\:)?((\d+\:(\d{1,2})\:(\d{1,2}))|((\d{1,2})\:(\d{1,2}))|(\d+))(\.\d*)?)?$/;
      if ((npt.test(start)) &&
          (npt.test(end))) {
        ...
        var convertToSeconds = function(time) {
          ...
          if (hours > 23) {
            ...
            logger.warn('Please ensure that hours <= 23.');
            return false;              
          }          
          if (minutes > 59) {
            // logWarning('Please ensure that minutes <= 59.');
            logger.warn('Please ensure that minutes <= 59.');
            return false;
          }
          if (seconds >= 60) {
            // logWarning('Please ensure that seconds < 60.');                      
            logger.warn('Please ensure that seconds < 60.');
            return false;
          }    
        ...
        if (start && end) {
          if (startNormalized < endNormalized) {
            ...
            logger.warn('Please ensure that start < end.');                                                      
            return false;            
          }           
        } else {
          if ((convertToSeconds(start) !== false) ||
            ...
            logger.warn('Please ensure that start or end are legal.');
            return false;
          }
        }
      }
      ...
      if ((smpte.test(start)) && (smpte.test(end))) {
        ...
        var convertToSeconds = function(time) {
          ...
          if (seconds > 59) {
            ...
            logger.warn('Please ensure that seconds <= 59.');                      
            return false;
          }    
          return hours * 3600 + minutes * 60 + seconds +
      ...
      logger.warn('Invalid time dimension.');                                                
      return false;
    },
    xywh: function(value) {
      // "pixel:" is optional
      ...
      if (pixelCoordinates.test(value)) {             
        if (w > 0 && h > 0) {
          ...
          logger.warn('Please ensure that w > 0 and h > 0');                                      
          return false;          
        }
        ...
        var checkPercentSelection = (function checkPercentSelection(
            x, y, w, h) {
          if (!((0 <= x) && (x <= 100))) { 
            logger.warn('Please ensure that 0 <= x <= 100.');                                      
            return false;
          }
          if (!((0 <= y) && (y <= 100))) { 
            logger.warn('Please ensure that 0 <= y <= 100.');                                      
            return false;
          }
          if (!((0 <= w) && (w <= 100))) { 
            logger.warn('Please ensure that 0 <= w <= 100.');                                      
            return false;
          }
          if (!((0 <= h) && (h <= 100))) { 
            logger.warn('Please ensure that 0 <= h <= 100.');                                      
            return false;
          }            
          return true;            
        });        
        ...
        logger.warn('Invalid percent selection.');                                      
        return false;
      } else {
        logger.warn('Invalid spatial dimension.');                                      
        return false;
      }
    },
  ...
  var splitKeyValuePairs = function(octetString) {
    /**
    * @dsm: adapted to include HTML fragment identifiers
    * 
    */
    var keyValues = {};
    var frag_ids = [];
    ...
    keyValuePairs.forEach(function(keyValuePair) {      
      ...
      if (position < 1) {
        // @dsm: html fragment identifier
        frag_ids.push(keyValuePair);
      } 
      var components = [
